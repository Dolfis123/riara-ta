...
const getStatistikPengambilan = async (req, res) => {
  try {
    ...
    if (startDate && endDate) {
      ...
      const startCustom = moment
        .tz(startDate, "Asia/Jayapura")
        .startOf("day")
        .toDate();
      const endCustom = moment
        .tz(endDate, "Asia/Jayapura")
        .endOf("day")
        .toDate();
    ...
    const customRangeCount =
      Object.keys(customRangeFilter).length > 0
...
const pengambilanBarang = async (req, res) => {
  ...
  try {
    ...
    if (Jumlah_Diambil > barang.Stok_Tersedia) {
      return res
        .status(400)
        .json({ message: "Jumlah diambil melebihi stok tersedia" });
    }
...
const getStatistikByDate = async (req, res) => {
  try {
    ...
    const startOfDay = moment
      .tz(tanggal, "YYYY-MM-DD", "Asia/Jayapura")
      .startOf("day")
      .toDate();
    ...
    res
      .status(500)
      .json({ message: "Gagal mengambil data berdasarkan tanggal." });
  }
};
...
const getRiwayatPengambilan = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Gagal mengambil data riwayat pengambilan." });
  }
};
module.exports = {
  pengambilanBarang,
