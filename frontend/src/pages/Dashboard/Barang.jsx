...
const Barang = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setBarangList(
        barangList.filter(
          (barang) => barang.ID_Barang !== barangToDelete.ID_Barang,
        ),
      );
  ...
  const handleModalSubmit = async (e) => {
    ...
    try {
      ...
      // Baris ini akan menangkap pesan "Nama barang sudah terdaftar..." dari backend
      const message =
        error.response?.data?.message || "Gagal menyimpan data barang.";
      setSuccessMessage(message); // Pesan error akan muncul di kotak notifikasi
